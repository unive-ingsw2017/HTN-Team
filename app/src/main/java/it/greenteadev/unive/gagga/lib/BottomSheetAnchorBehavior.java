...
public class BottomSheetAnchorBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private final List<View> mAnchoredViews = new ArrayList<>();
    ...
    public BottomSheetAnchorBehavior(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.BottomSheetAnchorBehavior);
        TypedValue peekValue = a.peekValue(R.styleable.BottomSheetAnchorBehavior_behavior_peekHeight);
        if (peekValue != null && peekValue.data == PEEK_HEIGHT_AUTO) {
            setPeekHeight(peekValue.data);
        } else {
            setPeekHeight(a.getDimensionPixelSize(
                    R.styleable.BottomSheetAnchorBehavior_behavior_peekHeight, PEEK_HEIGHT_AUTO));
        }
        setHideable(a.getBoolean(R.styleable.BottomSheetAnchorBehavior_behavior_hideable, false));
        setSkipCollapsed(a.getBoolean(R.styleable.BottomSheetAnchorBehavior_behavior_skipCollapsed,
                false));

        TypedValue anchorValue = a.peekValue(R.styleable.BottomSheetAnchorBehavior_behavior_anchorHeight);
        if (anchorValue != null && anchorValue.data == ANCHOR_HEIGHT_AUTO) {
            ...
            setAnchorHeight(a.getDimensionPixelSize(
                    R.styleable.BottomSheetAnchorBehavior_behavior_anchorHeight, ANCHOR_HEIGHT_AUTO));
        }

        setCollapsedColor(a.getColor(R.styleable.BottomSheetAnchorBehavior_behavior_collapsedColor, Color.WHITE));
        TypedValue anchorColorValue = new TypedValue();
        context.getTheme().resolveAttribute(R.attr.colorAccent, anchorColorValue, true);
        setAnchorColor(a.getColor(R.styleable.BottomSheetAnchorBehavior_behavior_anchorColor,
                anchorColorValue.data));

        setAnchorTextColor(a.getColor(R.styleable.BottomSheetAnchorBehavior_behavior_anchorTextColor, Color.WHITE));
        setCollapsedTextColor(a.getColor(R.styleable.BottomSheetAnchorBehavior_behavior_collapsedTextColor, Color.BLACK));

        if (a.hasValue(R.styleable.BottomSheetAnchorBehavior_behavior_header_layout)) {
            headerLayout = LayoutInflater.from(context).inflate(a.getResourceId(R.styleable.BottomSheetAnchorBehavior_behavior_header_layout, 0), null);
        ...
        if (a.hasValue(R.styleable.BottomSheetAnchorBehavior_behavior_content_layout)) {
            contentLayout = LayoutInflater.from(context).inflate(a.getResourceId(R.styleable.BottomSheetAnchorBehavior_behavior_content_layout, 0), null);
    ...
    public Parcelable onSaveInstanceState(CoordinatorLayout parent, V child) {
        return new SavedState(super.onSaveInstanceState(parent, child), mState, mHideable, mLocked);
    }
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (mState == STATE_EXPANDED) {
            ...
            if (mParallaxView != null) {
                int reference = savedTop - mParallaxView.getHeight();
                mParallaxView.setY(reference);
        ...
        if (nestedScrolling.getChildCount() == 0 &&
                headerLayout != null && contentLayout != null) {

            LinearLayout bottomsheet = new LinearLayout(mContext);
     ...
     * Useful for when you know the height of your mParallaxView but not the height of the parent.
     *
     ...
     * Useful for setting the height of your mParallaxView.
     *
     ...
     * Sets whether this bottom sheet can hide when it is swiped down.
     *
     * @param locked {@code true} to make this bottom sheet is locked in the current state.
     */
     ...
     * Gets whether this bottom sheet can hide when it is swiped down.
     *
     * @return {@code true} if this bottom sheet is locked in the current state.
     */
    ...
    /**
     * Anchor a view to the top of the bottomsheet/mParallaxView.
     *
    ...
    public void unanchorView(View view) {
        if (mAnchoredViews.contains(view)) {
            mAnchoredViews.remove(view);
        }
    }

    /**
     * Anchor the mMap to the bottomsheet. This will adjust the padding of the mMap into view
    ...
    private void setStateInternal(@State int state) {
        ...
        mState = state;
        // determine visibility of mParallaxView
        if (mParallaxView != null) {
            if (mState == STATE_COLLAPSED || mState == STATE_HIDDEN) {
                mParallaxView.setVisibility(View.GONE);
            } else {
                mParallaxView.setVisibility(View.VISIBLE);
            }
        }
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            int third = mHideable ? mParentHeight : mMaxOffset;
            return top < mMinOffset ? mMinOffset : (top > third ? third : top);
        }
    ...
    private void dispatchOnSlide(int top) {
        ...
        // move the mParallaxView relative to the bottomsheet and update colors
        if (mParallaxView != null) {
            int height = mParallaxView.getHeight();
            float y = mParallaxView.getY();
            if (slideOffset <= 0) {
                updateHeaderColor(mCollapsedColor, mCollapsedTextColor);
                mParallaxView.setVisibility(View.INVISIBLE);
            } else if (mAnchorOffset >= top && y <= mAnchorOffset - height && (y > 0 || top >= height)) {
                mParallaxView.setY(top - height);
            } else if (slideOffset > 0 && (top >= mAnchorOffset || y > mAnchorOffset - height)) {
                // math for translating mParallaxView relative to bottom sheet
                ...
                mParallaxView.setY(init + travelDistance * percentCovered);
            }
        }

        if (bottomSheet != null) {
            int reference;
            if (mParallaxView != null && slideOffset > 0) {
                reference = (int) mParallaxView.getY();
            } else {
    ...
    private void anchorViews(int reference) {
        ...
        for (int i = 0, size = mAnchoredViews.size(); i < size; i++) {
            View view = mAnchoredViews.get(i);
            CoordinatorLayout.LayoutParams lp = (CoordinatorLayout.LayoutParams) view.getLayoutParams();
            view.setY(reference - lp.bottomMargin - lp.height);
        }

        // set the padding on the mMap
        if (mMap != null) {
            int calcPaddingBottom = mParentHeight - reference;
            mMap.setPadding(mapPaddingLeft, mapPaddingTop, mapPaddingRight,
                    (calcPaddingBottom == 0) ? mapPaddingBottom : mParentHeight - reference);
        }
    }
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcel source, ClassLoader loader) {
            ...
            canHide = source.readInt() > 0;
            mLocked = source.readInt() > 0;
        }

        public SavedState(Parcelable superState, @State int state, boolean canHide, boolean locked) {
            super(superState);
            this.state = state;
        ...
        public void writeToParcel(Parcel out, int flags) {
            ...
            out.writeInt(state);
            out.writeInt(canHide ? 1 : 0);
            out.writeInt(mLocked ? 1 : 0);
        }
